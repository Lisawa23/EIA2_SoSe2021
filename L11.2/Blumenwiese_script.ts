namespace L11_2_Blumenwiese {
    ...
    function createFlower(): void {
        for (let i: number = 0; i < 7; i++) {
            let horizon: number = 600 * golden;
            let randomX: number = Math.floor(Math.random() * 800);
            let randomY: number = horizon + Math.floor(Math.random() * 250);
            let flowerRed: FlowerRed = new FlowerRed(randomX, randomY);
            flowers.push(flowerRed);
        }
        for (let i: number = 0; i < 7; i++) {
            let horizon: number = 600 * golden;
            let randomA: number = Math.floor(Math.random() * 800);
            let randomB: number = horizon + Math.floor(Math.random() * 250);
            let flowerBlue: FlowerBlue = new FlowerBlue(randomA, randomB);
            flowers.push(flowerBlue);
        }
        for (let i: number = 0; i < 7; i++) {
            let horizon: number = 600 * golden;
            let randomC: number = Math.floor(Math.random() * 800);
            let randomD: number = horizon + Math.floor(Math.random() * 250);
            let flowerPink: FlowerPink = new FlowerPink(randomC, randomD);
    ...
    function moreBees(): void {

        let h: Bee = new Bee(0.8); // Neue Biene mit neuer Klasse
        moveables.push(h);
    }
